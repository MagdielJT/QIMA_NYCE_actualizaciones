...
define(['N/log', 'N/ui/serverWidget', 'N/record', 'N/runtime', 'N/http', 'N/config', 'N/search', 'N/xml', 'N/file', 'N/render','N/format','N/url','N/redirect','N/transaction','N/https'],
    function (log, ui, record, runtime, http, config, search, xml, file, render, format,urlmod,redirect,transaction,https) {
            ...
            try {
                if (context.request.method === 'GET') {
                    section = 'Get Parameters';
                    {
                        ...
                        var motivo  = context.request.parameters.custparam_motivocancelacion || '';
                        ...
                        getNetSuite(context, scriptObj, tranid, trantype, searchTypeTran, cancelacionAutomatica,sustituir,solosustituir,motivo);
        ...
        function getNetSuite(context, scriptObj, tranid, trantype, searchTypeTran, cancelacionAutomatica,sustituir,solosustituir,motivo) {
            var errorConfig = [];
            var errorConfig_2 = [];
            var errorConfig_3 = [];
            ...
            if(solosustituir=='T'){
                ...
                xmlSatSend = xmlCancelSendSat(usuario_integrador, rfcEmisor, uuid,motivo);
                ...
                log.debug({title:'Error occurred in sending url',details:err});
                }
        ...
        function xmlCancelSendSat(usuarioIntegrador, rfcEmisor, uuid,motivo) {
            log.audit({ title: 'parametro xmlCancelSendSat', details: 'usuarioIntegrador: ' + usuarioIntegrador + ' rfcEmisor: ' + rfcEmisor + ' uuid: ' + uuid + 'motivo: ' + motivo });
            var xmlCancel = '';
            ...
            xmlCancel += '    <CancelaCFDI40 xmlns="http://tempuri.org/">';
            ...
            xmlCancel += '    <motivoCancelacion>' + motivo + '</motivoCancelacion>';
            xmlCancel += '    </CancelaCFDI40>';
            xmlCancel += '</soap12:Body>';
        ...
        function logError(section, err) {
            ...
            log.debug({ title: 'Error Notification on ' + section, details: err_Details });
        }
            ...
            for(var i=0;i<countInv;i++){
                ...
                if(factura_l) {
                    try{
                        ...
                        log.debug({ title: 'errotyipo ', details: errotyipo });
                        record.submitFields({
